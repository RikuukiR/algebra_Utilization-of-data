\documentclass[12pt,a4paper]{jsarticle}
\usepackage[margin=20truemm]{geometry}

% 節番号を「第1節」形式に
\renewcommand{\thesection}{第\arabic{section}節}

\input{preamble.tex}

% ========================================
% 解答の表示・非表示を切り替え
% ========================================
% 解答を表示する場合：\showanswertrue
% 解答を非表示にする場合：\showanswerfalse
\showanswertrue

% subsection番号を「1.1」形式に（thesectionの影響を受けないように）
\renewcommand{\thesubsection}{\arabic{section}.\arabic{subsection}}

% カウンターの初期値を設定
\setcounter{section}{0}
\setcounter{subsection}{0}
\setcounter{definition}{0}
\setcounter{theorem}{0}

% ========================================
% カウンターの階層化設定（○.○.○形式）
% ========================================
% 定義・定理・表カウンターをsubsectionに従属させる
\counterwithin{definition}{subsection}
\counterwithin{theorem}{subsection}
\counterwithin{table}{subsection}

% 表示形式を「○.○.○」に変更
\renewcommand{\thedefinition}{\arabic{section}.\arabic{subsection}.\arabic{definition}}
\renewcommand{\thetheorem}{\arabic{section}.\arabic{subsection}.\arabic{theorem}}
\renewcommand{\thetable}{\arabic{section}.\arabic{subsection}.\arabic{table}}

\title{【中学数学】 データの活用}                % タイトル
\author{}                     % 著者（空白でOK）
\date{}                       % 日付（空白）

\begin{document}
\begin{center}
{\LARGE 【中学数学】 第5章 データの活用}

体系数学2/数研出版/代数編
\end{center}

% --- 著者と日付を右寄せで表示 ---
\vspace{5mm}
\hfill Riku Sugawara \\
\hfill 12.2025

% ========================================
% 各セクションの読み込み
% ========================================

\input{sections/01_データの整理.tex}
\input{sections/02_データの代表値.tex}
% \input{sections/03_データの散らばりと四分位範囲.tex}

\end{document}
