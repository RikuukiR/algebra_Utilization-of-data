...
% 分数コマンドを再定義（分子と分母の両端に1文字分の空白を追加）
% 元のコマンドを保存
\let\oldfrac\frac
\let\olddfrac\dfrac
% 再定義（\; は1文字分の空白）
...
\newtcolorbox{definitionbox}[2][]{
    ...
    title={Definition \thedefinition\if\relax\detokenize{#2}\relax\else\quad#2\fi}
}
...
\newtcolorbox{theorembox}[2][]{
    ...
    title={Theorem \thetheorem\if\relax\detokenize{#2}\relax\else\quad#2\fi}
}
